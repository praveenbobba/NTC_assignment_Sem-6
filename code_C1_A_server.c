...
int main(void)
{
    int sockfd, new_fd,numbytes,count;  // listen on sock_fd, new connection on new_fd
    struct addrinfo hints, *servinfo, *p;
    struct sockaddr_storage their_addr; // connector's address information
    socklen_t sin_size;
    struct sigaction sa;
    int yes=1,k=0;
    char s[INET6_ADDRSTRLEN],buf[20],user[20],pass[20],log_user[20],log_pass[20];
    int rv,status = 0;
    FILE *fp, *fd;
    ...
    while(1) {  // main accept() loop
		...
		while(! feof(fp))
		{
			...
			if( strcmp(user,log_user)==0  && strcmp(pass,log_pass)==0 )
			{
				status = 1;
				break;
			}
